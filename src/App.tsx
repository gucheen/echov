...
export default function App() {
  ...
  const characters = useMemo(() => {
    return charactersData.异象回声角色tag.角色列表 as Character[];
  }, []);

  const stars = charactersData.异象回声角色tag.星级列表;
  const professions = charactersData.异象回声角色tag.职业列表;
    ...
    <div className="max-w-7xl mx-auto px-4 py-12 sm:px-6 lg:px-8">
      ...
      <footer className="mt-24 pt-8 border-t border-slate-200 text-center">
        <p className="text-slate-400 text-sm">
          &copy; {new Date().getFullYear()} Echov Archive • Data from Local Character Index
        </p>
      </footer>
    </div>
  );
