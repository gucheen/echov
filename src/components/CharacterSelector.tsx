...
export default function CharacterSelector({ onSelect, selectedCharacter }: CharacterSelectorProps) {
  ...
  return (
    <div className="relative w-full max-w-sm">
      ...
      >
        <span className={selectedCharacter ? "text-[#d4af37] font-black uppercase tracking-wider text-lg" : "text-slate-500 text-base"}>
        ...
        <div className="absolute z-[60] mt-1 w-full bg-[#1a1d23] border border-[#d4af37]/50 shadow-[0_10px_40px_rgba(0,0,0,0.5)] overflow-hidden">
          <div className="p-2 border-b border-slate-800">
            <input
              type="text"
              placeholder="搜索角色..."
              className="w-full px-3 py-3 bg-[#0f1115] border-none text-base text-white focus:ring-0 outline-none placeholder:text-slate-600"
              value={searchTerm}
          ...
          <div className="max-h-64 overflow-y-auto p-1 custom-scrollbar">
            <div 
              ...
              className="px-3 py-3 text-sm text-slate-500 hover:bg-white/5 cursor-pointer uppercase font-black tracking-tighter"
            >
              取消选择角色
            </div>
            {filteredCharacters.map((char) => (
              <div
                ...
                className={`px-3 py-4 text-base cursor-pointer flex items-center gap-3 transition-colors border-b border-white/5 last:border-none ${
                  selectedCharacter === char.角色名 
                    ? "bg-[#d4af37]/20 text-[#d4af37] font-black" 
                    : "text-slate-300 hover:bg-white/5 hover:text-white"
                }`}
              >
                <div className="w-8 h-8 bg-slate-800 flex items-center justify-center text-sm font-black border border-slate-700">
                  {char.角色名.charAt(0)}
                </div>
                <span className="uppercase tracking-tight">{char.角色名}</span>
                <span className="ml-auto text-sm text-slate-500 border border-slate-800 px-2 py-0.5 uppercase">
                  {char.星级[0]}
                </span>
              </div>
            ))}
