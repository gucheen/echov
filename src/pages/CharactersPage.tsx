...
const CharacterCard = ({ character }: { character: Character }) => {
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -5 }}
      className={cn(
        "metal-panel group overflow-hidden relative border-t-2 border-t-slate-700",
      )}
    >
      <div className="aspect-[3/4] overflow-hidden bg-slate-900 relative">
        ...
        <div className="absolute top-3 left-3 flex gap-1">
          {Array.from({
            length: character.星级[0].includes("六")
              ? 6
              : character.星级[0].includes("五")
                ? 5
                : 4,
          }).map((_, i) => (
            <Star
      ...
      <div className="p-4 space-y-3 bg-[#1a1d23]">
        ...
        <div className="flex flex-wrap gap-1.5">
          {character.功能?.slice(0, 3).map((tag: string, i: number) => (
            <span
              key={i}
              className="text-sm px-2 py-1 bg-[#d4af37]/5 text-[#d4af37] font-bold border border-[#d4af37]/20 uppercase"
            >
              {tag}
            </span>
          ))}
...
export default function CharactersPage() {
  ...
  const filteredCharacters = useMemo(() => {
    return characters.filter((char) => {
      const matchesSearch = char.角色名
        .toLowerCase()
        .includes(searchTerm.toLowerCase());
      const matchesStar = !selectedStar || char.星级.includes(selectedStar);
      const matchesProfession =
        !selectedProfession || char.职业.includes(selectedProfession);
    ...
    <div className="max-w-7xl mx-auto px-4 py-12 sm:px-6 lg:px-8">
      ...
      <header className="mb-12 text-center space-y-4">
        <motion.h1
          initial={{ opacity: 0, y: -20 }}
          animate={{ opacity: 1, y: 0 }}
          className="text-5xl font-black tracking-tight text-white sm:text-6xl uppercase"
        >
          角色档案 <span className="opacity-20 font-light">ARCHIVE</span>
        </motion.h1>
        <motion.p
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ delay: 0.2 }}
          className="text-base tracking-[0.2em] text-slate-400 max-w-2xl mx-auto uppercase font-bold"
        >
          轻松探索与筛选详尽的角色数据库。
        </motion.p>
      </header>
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <div className="relative flex-1 group">
            ...
            <input
              type="text"
              placeholder="按名称搜索..."
              className="w-full pl-12 pr-4 py-4 bg-[#0f1115] border border-[#404040] text-white text-lg focus:border-[#d4af37] outline-none transition-all placeholder:text-slate-600"
              value={searchTerm}
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="relative group">
              <select
                className="appearance-none pl-4 pr-10 py-4 bg-[#0f1115] border border-[#404040] text-slate-300 text-base focus:border-[#d4af37] outline-none cursor-pointer transition-all"
